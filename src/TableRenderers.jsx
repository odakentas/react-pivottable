...
function makeRenderer(opts = {}) {
    class TableRenderer extends React.PureComponent {
        render() {
            ...
            const totalsLabel =
                this.props.totalsLabel != null
                    ? this.props.totalsLabel
                    : 'Totals';
            ...
            if (grouping) {
                for (const key of folded) {
                    colKeys = colKeys.filter(
                        (colKey) =>
                            !flatKey(colKey).startsWith(
                                key + ROW_COL_KEY_JOINER
                            )
                    );
                    rowKeys = rowKeys.filter(
                        (rowKey) =>
                            !flatKey(rowKey).startsWith(
                                key + ROW_COL_KEY_JOINER
                            )
                    );
                }
            ...
            const rbClass = grouping
                ? this.props.rowGroupBefore
                    ? 'rowGroupBefore'
                    : 'rowGroupAfter'
                : '';
            const cbClass = grouping
                ? this.props.colGroupBefore
                    ? 'colGroupBefore'
                    : 'colGroupAfter'
                : '';
            const clickClass = (pred, closed) =>
                ...
                <table className={`pvtTable ${rbClass} ${cbClass}`}>
                    <thead>
                        {colAttrs.map(function (c, j) {
                            const clickable =
                                grouping && colAttrs.length > j + 1;
                            const levelKeys = colKeys.filter(
                                (x) => x.length === j + 1
                            );
                            return (
                                <tr key={`colAttr${j}`}>
                        ...
                        {rowAttrs.length !== 0 && (
                            <tr>
                                {rowAttrs.map(function (r, i) {
                                    const clickable =
                                        grouping && rowAttrs.length > i + 1;
                                    const levelKeys = rowKeys.filter(
                                        (x) => x.length === i + 1
                                    );
                                    return (
                                        <th
                        ...
                        {rowKeys.map(function (rowKey, i) {
                            ...
                            return (
                                <tr
                                    key={`rowKeyRow${i}`}
                                    className={
                                        rowGap ? 'pvtLevel' + rowGap : 'pvtData'
                                    }
                                >
    ...
    TableRenderer.defaultProps.tableColorScaleGenerator =
