...
export class DraggableAttribute extends React.Component {
    ...
    getFilterBox() {
        ...
        return (
            <Draggable handle=".pvtDragHandle" nodeRef={dragRef}>
                <div
...
class PivotTableUI extends React.PureComponent {
            ...
            >
                {lclItems.map((x) => (
                    <DraggableAttribute
                        ...
                        removeValuesFromFilter={this.removeValuesFromFilter.bind(
                            this
                        )}
                        zIndex={
                            this.state.zIndices[x.name] || this.state.maxZIndex
                        }
                    />
                ))}
            </ReactSortable>
    ...
    render() {
        const numValsAllowed =
            this.props.aggregators[this.props.aggregatorName]([])().numInputs ||
            0;
        ...
        const rendererCell = (
            <td className="pvtRenderers">
                <Dropdown
                    ...
                    zIndex={
                        this.isOpen('renderer') ? this.state.maxZIndex + 1 : 1
                    }
                    toggle={() =>
                        this.setState({
                            openDropdown: this.isOpen('renderer')
                                ? false
                                : 'renderer'
                        })
                    }
        ...
        const aggregatorCell = (
            <td className="pvtVals">
                <Dropdown
                    ...
                    zIndex={
                        this.isOpen('aggregators')
                            ? this.state.maxZIndex + 1
                            : 1
                    }
                    toggle={() =>
                        this.setState({
                            openDropdown: this.isOpen('aggregators')
                                ? false
                                : 'aggregators'
                        })
                    }
                ...
                {new Array(numValsAllowed).fill().map((n, i) => [
                    <Dropdown
                        ...
                        toggle={() =>
                            this.setState({
                                openDropdown: this.isOpen(`val${i}`)
                                    ? false
                                    : `val${i}`
                            })
                        }
