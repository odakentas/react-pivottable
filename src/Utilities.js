                    ...
                    push(record) {
                        ...
                        if (['min', 'max'].includes(mode)) {
                            ...
                            if (!isNaN(x)) {
                                this.val = Math[mode](
                                    x,
                                    this.val !== null ? this.val : x
                                );
                            }
                        }
                        if (
                            mode === 'first' &&
                            this.sorter(x, this.val !== null ? this.val : x) <=
                                0
                        ) {
                            this.val = x;
                        }
            ...
            return function () {
                return {
                    ...
                    value() {
                        ...
                        return (
                            (this.vals[Math.floor(i)] +
                                this.vals[Math.ceil(i)]) /
                            2.0
                        );
                    },
                    format: formatter,
...
const zeroPad = (number) => `0${number}`.slice(-2); //.substr(-2, 2);
...
class PivotData {
    ...
    getUserSortedRowKeys(columnKey, sortOrder) {
        try {
            ...
            return [...rowKeys].sort((rowAKeySet, rowBKeySet) => {
                ...
                const rowAValue =
                    tree[rowAKey][columnKey] || FALLBACK_ROW_VALUE;
                const rowBKey = rowBKeySet.join(ROW_COL_KEY_JOINER);
                const rowBValue =
                    tree[rowBKey][columnKey] || FALLBACK_ROW_VALUE;
                return (
                    sortMultiplier *
                    naturalSort(rowAValue.value(), rowBValue.value())
                );
            });
        } catch (e) {
            return this.rowKeys;
    ...
    sortKeys() {
        if (!this.sorted) {
            ...
            switch (this.props.rowOrder) {
                ...
                case 'value_z_to_a':
                    this.rowKeys.sort(
                        (a, b) => -naturalSort(v(a, []), v(b, []))
                    );
                    break;
                default:
                    this.rowKeys.sort(
                        this.arrSort(this.props.rows, this.props.rowGroupBefore)
                    );
            }
            switch (this.props.colOrder) {
                ...
                default:
                    this.colKeys.sort(
                        this.arrSort(this.props.cols, this.props.rowGroupBefore)
                    );
            }
        }
    }
    ...
    getColKeys(all_keys = false) {
        ...
        return all_keys
            ? this.colKeys
            : this.colKeys.filter((x) => x.length === this.props.cols.length);
    }
    ...
    getRowKeys(all_keys = false) {
        ...
        return all_keys
            ? this.rowKeys
            : this.rowKeys.filter((x) => x.length === this.props.rows.length);
    }
